...
- Merging to `main` triggers `.github/workflows/release.yml`. `scripts/release.ts` consumes pending changesets, updates `package.json` and `CHANGELOG.md`, builds, commits the release, tags it, publishes to npm, and creates the GitHub Release. It exits without publishing when no changesets are pending.
- Before releasing, run `pnpm release:dry`; never use `pnpm release` locally unless the user explicitly asks to publish and create the release.
